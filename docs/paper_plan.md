...
**From Detected Misstatements to Review and Correction: A Filing-Origin Measure of Reporting Risk**
...
- **Empirical object.** The paper studies a filing-origin, submission-dated set of review-and-correction outcomes built from SEC and PCAOB records.
- **Primary target.** The outcomes measure subsequently recorded correspondence, filing friction/correction, and non-reliance events, not unobserved fraud occurrence or correspondence availability on its submission date.
...
- **Estimand shift.** Prior fraud and restatement studies often predict detected ex post misconduct labels; this paper predicts subsequent submission-dated correspondence and public correction events from a filing-origin information set.
...
- **Positioning.** The paper aligns the prediction target to explicit regulatory-record event clocks while separating correspondence submission from public release.
...
- **Overlap role.** The bridge tests where the recorded outcomes agree or disagree with detected-misstatement labels.
...
- **What prior results cannot support.** Prior results do not make the recorded outcomes equivalent to fraud, and they do not make the outcome task a same-estimand benchmark against detected-misstatement classifiers.
...
- **Gap 2: recorded review process.** SEC comment-letter and disclosure-review studies establish that disclosed correspondence is economically meaningful, but they usually do not build a multi-outcome, filing-origin prediction design that distinguishes correspondence submission from public release.
...
- **Core question.** Can reporting-risk prediction be reframed from ex post detected misconduct to filing-origin review-and-correction outcomes, with SEC correspondence dated by the underlying EDGAR submission rather than an unverified release date?
...
- **Filing-origin estimand.** The repo defines a filing-origin, submission-dated review-and-correction estimand. Predictors must be verifiably available at or before `origin_date`; the comment-thread outcome is eventually disclosed correspondence dated by its underlying EDGAR submission date and is not claimed to have been public on that date.
- **Construct claim.** The review-and-correction outcomes are expected to be related to, but not identical with, detected-misstatement benchmark labels.
...
- **Contribution.** The paper develops a measurement-and-ranking framework for filing-origin review-and-correction outcomes recorded in regulatory infrastructure and a transparent construct comparison with the detected-misstatement benchmark.
...
- **Boundary for results.** Results should be interpreted as evidence about submission-dated correspondence and public correction outcomes rather than unobserved fraud occurrence, causal effects, correspondence availability on the submission date, or full SEC-review coverage.
...
- **Information set.** The public-cascade predictor set is filing-origin: every predictor must be verifiably available at or before `origin_date`. Submission-dated comment-thread history is excluded because its public-release timing is not verified.
...
flowchart LR
    ...
    subgraph PUBLIC["Public filing-origin cascade: SEC/PCAOB public information set"]
        P0["Public-source inputs<br/>EDGAR filings, FSDS/XBRL, Notes summaries,<br/>eventually disclosed correspondence submissions,<br/>amendments, 8-K Item 4.02, PCAOB Form AP<br/>sample 2011-2024, as-of 2026-07-06"]
        ...
        P3["Filing-origin X<br/>metadata, XBRL ratios, auditor, prior-filing history, all<br/>only verifiably origin-date-available fields<br/>submission-dated comment history excluded<br/>exclude source_available_*, public_date_*, vintage_* fields"]
        P4["Recorded Y<br/>label_comment_thread_365: submission-dated SEC correspondence<br/>label_amendment_365: amended filing or filing friction<br/>label_8k_402_365: Item 4.02 non-reliance"]
        P5["Public-side prediction loops<br/>complete origin-calendar-year holdouts<br/>training outcomes mature before each holdout begins<br/>rolling/expanding windows from earlier origin years<br/>core model plus public-label peer suite"]
        ...
        P7["Interpretation<br/>filing-origin submission-dated outcome ranking<br/>eventually disclosed correspondence plus public correction events<br/>not a correspondence-release-time backtest"]
...
- **Source-to-table mapping.**
    ...
    - SEC `UPLOAD` and `CORRESP` produce `comment_thread.csv.gz` with first and last underlying correspondence submission dates. The table does not contain public-release dates.
...
- **Outcome grain.** Outcome labels are attached to the annual `issuer_cik x fiscal_year x origin_date` issuer-year row.
- **Outcome design.** The three labels form a multi-outcome system, not a deterministic hierarchy.
- **Outcome labels.**
    - `label_comment_thread_365`: eventually disclosed SEC correspondence, grouped into threads and dated by the first underlying EDGAR submission date. This is not a public-release date and not the full SEC review universe; source: [SEC filing review process](https://www.sec.gov/about/divisions-offices/division-corporation-finance/filing-review-process-corp-fin) and EDGAR correspondence submissions.
...
- **Forward horizons.**
    - `label_comment_thread_365 = 1` if the first submission date in an eventually disclosed SEC correspondence thread falls after `origin_date` and within 365 days.
...
- **Construct meaning.** These labels are not alternative names for fraud. They are typed recorded outcomes: eventually disclosed regulatory correspondence (`comment_thread`), filing correction or friction (`amendment`), and material non-reliance (`8k_402`).
- **Target distinction.** The target is a submission-dated correspondence and public-correction outcome state rather than unobserved fraud occurrence or real-time correspondence availability.
...
- **Pretreatment target.** All preprocessing is designed to preserve the filing-origin predictor set: transform raw public-source tables into issuer-year features without introducing any post-`origin_date` information or correspondence fields whose release-time availability cannot be verified.
...
- **Filing friction and dated history.** Current-cycle NT status and amendment friction, plus strictly filing-origin-available rolling counts for prior NT filings, amendments, and 8-K instability items. Comment-thread history based only on correspondence submission dates is retained for descriptive selection profiles but excluded from predictors because submission before `origin_date` does not establish public release before `origin_date`.
...
- **Auditor and prior-filing history.** PCAOB Form AP may supply auditor and engagement-partner features. The prior-filing-history family means `prior_filing_count`, not PCAOB inspection. PCAOB inspection archives are provenance inputs only; current inspection events are not joined to the analytical panel, and there are no inspection model features.
- **Note opacity.** Note count, note character count, note-tag coverage, and tag entropy as a disclosure breadth measure.
- **Feature-family boundary.** The reported public families are metadata, XBRL, auditor, prior-filing history, visibility/history, and all. The notes/disclosure-breadth variables enter `all`; there is no standalone text-family ablation.
...
- **Information set.** The `visibility_history` family is a compact visibility/history information set comprising filing size and type, XBRL flags, filing persistence, filing-origin friction, and one- and three-year histories of amendments and Items 3.01, 4.01, 4.02, and 5.02. Submission-dated comment-thread histories are excluded.
...
- **Split design.** Public-side prediction experiments use complete origin-calendar-year holdouts, not random cross-validation or fiscal-year grouping. Training origin years are separated from each holdout by enough time for the 365-day submission-dated outcome window to mature before January 1 of the holdout year. This enforces temporal ordering under the chosen event clock; it is not a release-time backtest for correspondence.
...
| Public cascade core | XGBoost classifier over metadata, XBRL, auditor, prior-filing history, visibility/history, and all feature sets; notes/disclosure-breadth variables enter `all` only | `issuer_origin_panel` rows with verifiably filing-origin-available features | Main filing-origin submission-dated review-and-correction prediction task. |
...
| Public-label opacity DML | Double / Debiased Machine Learning (DML) partially linear regressions with issuer-group cross-fitted nuisance models and issuer-clustered covariance | `missingness_density_score` and filing-origin controls | Adjusted association between opacity/missingness and public labels, not a causal effect. |
...
- **Intervals and exploratory scope.** Construct-alignment intervals use seed 42 and 1,000 issuer-cluster percentile bootstrap draws. The interval scope is the primary plus the top five exploratory rows in each direction; exploratory maxima are labeled as such and remain outside the primary claim.

### Performance Metrics and Selection Criteria

#### Metric Selection Criteria

- **Primary criterion.** PR-AUC relative to prevalence is the first-read metric because the outcomes are imbalanced and ranking recorded review-and-correction cases is the main empirical task.
...
- **Annual support transparency.** Table 3 reports evaluation-period averages, while the fold-support appendix shows every origin-calendar test year and its positive count. No single year is designated ex post as the preferred exclusion sensitivity.
...
- **Purpose.** Test whether filing-origin opacity and missingness profiles are associated with later recorded scrutiny or correction outcomes.
- **Design.** Construct missingness-density and missing-profile indicators; estimate Double / Debiased Machine Learning (DML) partially linear regressions on public labels using issuer-group cross-fitting and issuer-clustered covariance.
...
- **Controls.** `X = filing-origin metadata, XBRL, filing-friction, verifiably dated amendment/8-K history, auditor, prior-filing history, note-opacity, and calendar controls`. Submission-dated correspondence history is excluded.
...
- **Design.** Build the public lake from SEC/PCAOB sources; construct the correspondence label from the first submission date in each eventually disclosed thread and correction labels from filing dates; report source coverage, event rates, censoring, and task readiness.
...
- **Interpretation.** This experiment validates the measurement surface for submission-dated correspondence and public correction states; it does not validate correspondence availability on the submission date.

### Experiment 5: Public Cascade Prediction

- **Purpose.** Estimate the filing-origin, submission-dated review-and-correction outcome state from verifiably origin-date-available features.
- **Design.** Use `issuer_origin_panel` to predict submission-dated correspondence, broad amendment/friction, and 8-K Item 4.02 outcomes. The headline uses the revision-frozen `all + expanding` specification; metadata, XBRL, auditor, prior-filing history, visibility/history, all-feature, and training-window grids are sensitivities.
...
- **Outcome-model expectation.** Filing-origin features should predict later submission-dated correspondence and public correction outcomes above each task's prevalence baseline, especially for correspondence and amendment/friction labels.
- **Feature-family expectation.** The reported grid contains metadata, XBRL, auditor, prior-filing history, visibility/history, and all; notes/disclosure-breadth variables enter `all` only, with no standalone text-family ablation.
...
> The outcome design supports evidence about a filing-origin, submission-dated review-and-correction state. It does not by itself identify unobserved fraud occurrence, causal effects, correspondence availability on the submission date, or a stable enforcement-prediction result.

> Correspondence threads are submission-dated outcomes disclosed later, not observations of the full SEC review universe or of public availability on the submission date.
...
- Prior-filing history means `prior_filing_count`, not PCAOB inspection.
...
| Public cascade | Require the frozen primary rows, task eligibility, complete origin-calendar-year support, mature training labels, and reported one-class skips. |
...
- **Paper-readiness gates.**
    ...
    - Correspondence is described as submission-dated and disclosed later, not as complete SEC review or contemporaneously public scrutiny.
